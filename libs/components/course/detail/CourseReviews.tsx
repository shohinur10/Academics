...
import { getCourseReviews, getRatingDistribution } from '../../../mock/courses.mock';
...
const CourseReviews = ({ course }: CourseReviewsProps) => {
	...
	const reviews = getCourseReviews(course._id);
		...
		<div className={'course-reviews'}>
			<div className={'reviews-summary'}>
				<div className={'overall-score'}>
					<strong>{course.courseRating?.toFixed(1)}</strong>
					<Stars rating={course.courseRating ?? 0} />
					<span>
						{course.courseReviewsCount} {t('Reviews')}
					</span>
				</div>
					...
					{distribution.map((percent, index) => (
						<div key={index} className={'distribution-row'}>
							...
							<div
								className={'bar'}
								role="img"
								aria-label={`${5 - index} stars: ${percent}%`}
							>
